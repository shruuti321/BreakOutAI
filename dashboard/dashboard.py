...
#
#             # Generate and display customized queries
...
#
#             # Store or send `generated_queries` to the backend or next step as needed
#
...
#
#             # Store or send `generated_queries` to the backend or next step as needed
# if st.button("Perform Web Search for Queries"):
...
#     return response.json()
#
# # Add the UI elements for running the Groq processing
# # if st.button("Process Data with Groq"):
# #     entity_data = [
# #         {"name": row["name"], "company": row["company"]}
# #         for _, row in pd.DataFrame(result['preview']).iterrows()
# #     ]
# #
# #     prompt_template = st.text_input("Enter your prompt template:", "From the search results, extract the email address for {company} and return only the email address in plain text."
# # )
# #
# #     groq_result = process_data_with_groq(entity_data, prompt_template)
# #
# #     # Display Results
...
# #         )
# if st.button("Process Data with Groq"):
#     # Ensure search results are added to entity_data
#     entity_data = []
#     for query in generated_queries:
#         company_name = extract_company_from_query(query)  # Custom function to extract company name
#         search_results_for_company = search_results.get(query, {}).get("results", [])
#
...
#
#     # Send entity_data with search results to Groq
#     groq_result = process_data_with_groq(entity_data, prompt_template)
#
...
if st.button("Process Data with Groq") and 'search_results' in st.session_state:
    ...
    # entity_data = []
    # extracted_emails = []  # Store extracted emails
    #
    # for query in generated_queries:
    #     company_name = extract_company_from_query(query)  # Custom function to extract company name
    ...
    #     for result in st.session_state.search_results.get(query, []):  # Ensure to handle query-specific results
    #         snippet = result.get('snippet', '')
    #
    #         # Extract emails from the snippet
    #         emails_in_snippet = extract_email_from_snippet(snippet)
    #         if emails_in_snippet:
    #             extracted_emails.extend(emails_in_snippet)
    #
    #         search_results_for_company.append(result)
    #
    #     # Log the search results for the company
    #     st.write(f"Search results for company '{company_name}':", search_results_for_company)
    #
    #     # Combine company info and search results
    #     entity_data.append({
    #         "company": company_name,
    ...
    # groq_result = {"results": [{"company": company_name, "emails": extracted_emails} for company_name in
    #                            [data["company"] for data in entity_data]]}
    #
    # # Display extracted information
    # st.write("Extracted Information:")
    # if 'results' in groq_result:
